(ns matterport-api.main
  "Kotodama WASM entrypoint for the Matterport_api clean-room actor (L4) — Clojure port.
  ...
  facts (`matterport_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "matterport_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "Anchor"   :plural "anchors"   :id-prefix "matterpo_anc"
...
(defn emit-facts
  "EAVT facts for one record: {\"matterport_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "matterport_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :matterport_api-compat/ready)
